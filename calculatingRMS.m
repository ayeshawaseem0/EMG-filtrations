...
% Calculate the RMS values for different conditions
rmsRelaxedBeforeFiltering = rms(emgRelaxed);
rmsContractedBeforeFiltering = rms(emgContracted);
rmsRelaxedAfterFiltering = rms(filteredRelaxed);
rmsContractedAfterFiltering = rms(filteredContracted);

% Display the RMS values
disp('RMS Values:');
disp(['Relaxed Before Filtering: ', num2str(rmsRelaxedBeforeFiltering)]);
disp(['Contracted Before Filtering: ', num2str(rmsContractedBeforeFiltering)]);
disp(['Relaxed After Filtering: ', num2str(rmsRelaxedAfterFiltering)]);
disp(['Contracted After Filtering: ', num2str(rmsContractedAfterFiltering)]);
numerator = b;
denominator = a;
Fs = 2047.8; % Sampling frequency in Hz
